...
export class GTE extends Operator {
    ...
    static apply(
        ...
        if (Array.isArray(value) && Array.isArray(search)) {
            ...
            return value === true || search === false; // GTE for boolean is not standard, but can be interpreted as true >= false
        } else {
            throw new Error("Invalid types for GTE operator");
        }
    }
